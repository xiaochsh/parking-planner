...
function [X, Y, Theta] = WayPointsOnClothoid(x0, y0, theta0, kappa, dkappa, L, npts, reverse)
    X = zeros(1, npts, 'single');
    Y = zeros(1, npts, 'single');
    Theta = zeros(1, npts, 'single');
    for m = 1 : npts
        t = single(m - 1) * L / single(npts - 1);
        if reverse
            [C, S] = GeneralizedFresnelCS(1, dkappa*t^2, kappa*t, theta0 + single(pi)) ;
        else
            [C, S] = GeneralizedFresnelCS(1, dkappa*t^2, kappa*t, theta0) ;
        end
        X(m) = x0 + t * C ;
        Y(m) = y0 + t * S ;
        Theta(m) = theta0 + 0.5 * dkappa * t ^ 2 + kappa * t ;
    end
end
